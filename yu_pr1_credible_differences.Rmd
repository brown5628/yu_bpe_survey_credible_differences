...
```
```{r, include=FALSE}
# On new setup needs Snowflake ODBC driver installed.
# Credentials stored in OS credential storage per https://db.rstudio.com/best-practices/managing-credentials/

snow <- DBI::dbConnect(
  drv = odbc::odbc(), 
  UID = keyring::key_list("snowflake")[1,2],
  PWD = keyring::key_get("snowflake", (keyring::key_list("snowflake")[1,2])),
  Server = 'yearup.us-east-1.snowflakecomputing.com',
  Warehouse = "QUERY",
  Role = 'SYSADMIN',
  Database = 'TEST_DB',
...
```{sql, connection=snow, output.var="pr1_raw"}
...
from analytics.program.performance_review_1_results_materialized as pr1
...
```{r}
# make snowfalke stop yelling

colnames(pr1_raw) <- tolower(colnames(pr1_raw))
...
#h <- 1
  ...
  results_hist_l3[nrow(results_hist_l3) + 1, ] <- list(ymd(l), '_hist_l3', m, n, temp_results_hist_l3[1], temp_results_hist_l3[2], temp_results_hist_l3[3])
}
```
